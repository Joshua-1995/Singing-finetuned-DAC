...
**Largest-improvement examples** (per-clip, via [`scripts/pick_samples.py`](scripts/pick_samples.py)):

![highlight samples](docs/highlight_samples.png)

*Top: biggest Mel-distance drop (0.95 → 0.43). Bottom: biggest F0-RMSE drop
(490 → 73 cents) — the pretrained model badly distorted the pitch/harmonics here, the
fine-tuned model restores them.*

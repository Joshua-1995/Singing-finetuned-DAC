# ---------------------------------------------------------------------------
# Adapted from the Descript Audio Codec (DAC) training script.
#   Source: https://github.com/descriptinc/descript-audio-codec  (MIT, (c) Descript)
# Modification: in train_loop/val_loop, recon and target are trimmed to a common
# length before the losses, so a segment length that is an exact multiple of
# hop_length does not cause a shape mismatch. See docs/PATCHES.md.
# Two further patches live in third-party packages (argbind, audiotools) and must
# be applied separately — see docs/PATCHES.md.
# ---------------------------------------------------------------------------
import os
import sys
import warnings
...
def val_loop(batch, state, accel):
    ...
    # Align lengths: when the segment length is an exact multiple of hop_length the
    # decoder can return a few fewer samples than the input, which breaks the
    # spectral losses. Trim both to the common length.
    _n = min(out["audio"].shape[-1], signal.audio_data.shape[-1])
    signal.audio_data = signal.audio_data[..., :_n]
    recons = AudioSignal(out["audio"][..., :_n], signal.sample_rate)
...
def train_loop(state, batch, accel, lambdas):
    ...
    with accel.autocast():
        out = state.generator(signal.audio_data, signal.sample_rate)
        # Align lengths (see val_loop): decoder may return fewer samples when the
        # segment length is an exact multiple of hop_length.
        _n = min(out["audio"].shape[-1], signal.audio_data.shape[-1])
        signal.audio_data = signal.audio_data[..., :_n]
        recons = AudioSignal(out["audio"][..., :_n], signal.sample_rate)
